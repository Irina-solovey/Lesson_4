...
class Train
  ...
  attr_reader :speed, :train_car, :number, :type, :current_station, :previous_station, :next_station, :route, :index
  ...
  def previous_station
    @index = @route.stations.index(@current_station)  # Находим индекс текущей станции
    @previous_station = @route.stations[@index - 1]   # Возвращаем предыдущую станцию (текущая осталась той же)
  end
  def next_station
    @index = @route.stations.index(@current_station)  # Находим индекс текущей станции
    @next_station = @route.stations[@index + 1]   # Возвращаем следующую станцию (текущая осталась той же)
  end
  def current_station
    @index = @route.stations.index(@current_station)  # Находим индекс текущей станции
    @current_station = @route.stations[@index]  # Возвращаем текущую станцию
  end
end
